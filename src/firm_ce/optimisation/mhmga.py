...
from firm_ce.optimisation.st_solution import Solution, evaluate, extract_operations


@njit(parallel=True)
def mga_wrapper_with_operations(
    xs: nbfloat[:, :],
    ...
    for j in prange(n_points):
        ...
        extract_operations(solution, ops_data[niche_idx, j])
