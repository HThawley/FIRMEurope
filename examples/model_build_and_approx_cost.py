...
from firm_ce.optimisation.single_time import Solution, evaluate_from_operations
...
def evaluate_points(points, operations, static, fleet, network, balancing_type, fixed_costs_threshold):
    ...
    for i in prange(points.shape[0]):
        ...
        evaluate_from_operations(solution, operations[i])
...
def approx_costs(scenario, config):
    ...
    if not pop_path:
        ...
        pop_temp = True
    ops_path = _try_path(scenario.solution_dir, "latest_operations.csv")
    if not ops_path:
        ops_path = _try_path("results/temp", "latest_operations.csv")
        ops_temp = True
    if not pop_path or not ops_path:
        ...
        raise FileNotFoundError(f"Did not find all required files. Found 'latest_population.csv'={bool(pop_path)}."
                                f"Found 'latest_operations.csv'={bool(ops_path)}.")
    if pop_temp != ops_temp:
        print("Warning: found only one of 'latest_population.csv' and 'latest_operations.csv'"
    ...
    points = points[:, 3:]
    operations = pd.read_csv(ops_path, header=None).to_numpy().astype(npfloat)
